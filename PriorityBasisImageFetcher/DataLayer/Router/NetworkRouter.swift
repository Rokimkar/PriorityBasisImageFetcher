...
class NetworkRouter{
    ...
    func fetchDataFromNetwork (for request : EndPointType,success : @escaping response<Data>,failure : @escaping (Error)->Void){
        if let urlPath = request.completeUrl{
            Alamofire.request(urlPath, method: HTTPMethod.get, parameters: nil, encoding: JSONEncoding.default, headers: request.headers).responseData { (data) in
            ...
            failure(NSError.init(domain: "Not a valid url", code: 0, userInfo: [:]))
        }
    }
